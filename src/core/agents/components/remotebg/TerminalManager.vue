  ...
  <q-dialog v-model="showCustomShellDialog" persistent>
    <q-card style="min-width: 400px">
      <q-form @submit.prevent="startCustomShell">
        ...
        <q-card-section>
          <q-input v-model="customShellInput" label="Shell Path" dense autofocus />
        </q-card-section>
...
import { useTerminalWSConnection } from "src/websocket/websocket";
import "@xterm/xterm/css/xterm.css";

import type { TerminalDefaults } from "../../types";
...
function cleanupCurrentSession({ sendKill = true }: { sendKill?: boolean } = {}) {
...
function initWS(shell: string) {
  ...
  ({
    ...
    close: wsClose,
    status: wsStatus,
  } = useTerminalWSConnection(props.agentId, sessionId) as {
...
function onShellChange(newShell: string) {
  if (!term) return;
...
const windowsThemeVariant = computed<"cmd" | "powershell" | "pwsh7">(() => {
  ...
  if (selectedShell.value === "custom" && customShellPath.value) {
    if (/(?:^|[\\/])pwsh(?:-preview)?(?:\.exe)?$/i.test(customShellPath.value)) {
      return "pwsh7";
    }
  }
...
onMounted(() => {
  ...
  const { stop } = useResizeObserver(xtermContainer, () => void resizeWindow());
  ...
  const shellToStart =
    selectedShell.value === "custom" ? customShellPath.value! : selectedShell.value;

  initWS(shellToStart);
});

function onRunAsChange() {
  if (!term) return;
